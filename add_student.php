 ...
 <?php
   ...
   if(isset($_POST['submit']))
   {
     if(checkStudentId())
     {
       ...
       $sql = "INSERT INTO          tb_student(name,email,phone,address,section,student_id,image,user_name,entry_time)values('$name','$email','$phone','$address','$section','$student_id','$dst_db','$user_name','$current_datetime')";
   ...
   {
     $db     = new Database();
     $sql    = "SELECT * FROM tb_student WHERE student_id='".$_POST['student_id']."'";
     $result = $db->link->query($sql) or die($this->link->error.__LINE__);
     if($result->num_rows > 0)
     {
       ?>
        <script type="text/javascript">
          window.alert("Warning! Student_Id already Exist.");
        </script>
       <?php
       return false;
     }
